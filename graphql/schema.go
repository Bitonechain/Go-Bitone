...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte Bitone address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an Bitone account at a particular block.
    type Account {
    ...
    # Transaction is an Bitone transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an Bitone account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an Bitone account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an Bitone block by number or by hash. If neither is
