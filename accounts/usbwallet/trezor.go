...
import (
	...
	"github.com/bitone/go-bitone/accounts/usbwallet/trezor"
...
// or whether the Bitone app was not started on it.
...
// Bitone address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.BitoneAddress)
	if _, err := w.trezorExchange(&trezor.BitoneGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	response := new(trezor.BitoneTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.BitoneTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
