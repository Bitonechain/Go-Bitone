...
import (
	...
	"github.com/bitone/go-bitone/accounts/external"
...
func newClient(ctx *cli.Context) *bitoneclient.Client {
	client, err := bitoneclient.Dial(ctx.GlobalString(nodeURLFlag.Name))
	if err != nil {
		utils.Fatalf("Failed to connect to Bitone node: %v", err)
	}
	return client
}
...
func newContract(client *rpc.Client) (common.Address, *checkpointoracle.CheckpointOracle) {
	...
	contract, err := checkpointoracle.NewCheckpointOracle(addr, bitoneclient.NewClient(client))
	if err != nil {
