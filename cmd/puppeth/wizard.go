...
type config struct {
	...
	bitonestats  string   // Ethstats settings to cache for node deploys
...
type wizard struct {
	...
	services map[string][]string   // Bitone services known to be running on servers
...
// converts it to an Bitone address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by bitonestats, which compares IPs textually, not by value.
