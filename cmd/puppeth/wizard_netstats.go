...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for bitonestats availability")
	if infos, err := checkEthstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["bitonestats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["bitonestats"] = infos.Report()
		bitonestats = infos.config
	}
	...
	if bitonestats != "" {
		w.conf.bitonestats = bitonestats
	}
