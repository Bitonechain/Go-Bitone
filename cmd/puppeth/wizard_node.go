...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.bitonestats == "" {
		log.Error("No bitonestats server configured")
		return
	}
	...
	fmt.Println()
	if infos.bitonestats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.bitonestats = w.readString() + ":" + w.conf.bitonestats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.bitonestats)
		infos.bitonestats = w.readDefaultString(infos.bitonestats) + ":" + w.conf.bitonestats
	}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy Bitone node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
