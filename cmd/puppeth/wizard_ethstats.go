...
// deployEthstats queries the user for various input on deploying an bitonestats
...
func (w *wizard) deployEthstats() {
	...
	// Retrieve any active bitonestats configurations from the server
	infos, err := checkEthstats(client, w.network)
	if err != nil {
		infos = &bitonestatsInfos{
			port:   80,
			host:   client.server,
			secret: "",
		}
	...
	fmt.Println()
	fmt.Printf("Which port should bitonestats listen on? (default = %d)\n", infos.port)
	infos.port = w.readDefaultInt(infos.port)

	// Figure which virtual-host to deploy bitonestats on
	if infos.host, err = w.ensureVirtualHost(client, infos.port, infos.host); err != nil {
		log.Error("Failed to decide on bitonestats host", "err", err)
		return
	}
	// Port and proxy settings retrieved, figure out the secret and boot bitonestats
	fmt.Println()
	if infos.secret == "" {
	...
	// Try to deploy the bitonestats server on the host
	nocache := false
	if existed {
		fmt.Println()
		fmt.Printf("Should the bitonestats be built from scratch (y/n)? (default = no)\n")
	...
	if out, err := deployEthstats(client, w.network, infos.port, infos.secret, infos.host, trusted, infos.banned, nocache); err != nil {
		log.Error("Failed to deploy bitonestats container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
