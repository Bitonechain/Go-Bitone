...
func (w *wizard) deployExplorer() {
	...
	fmt.Println()
	if infos.node.bitonestats == "" {
		fmt.Printf("What should the explorer be called on the stats page?\n")
		infos.node.bitonestats = w.readString() + ":" + w.conf.bitonestats
	} else {
		fmt.Printf("What should the explorer be called on the stats page? (default = %s)\n", infos.node.bitonestats)
		infos.node.bitonestats = w.readDefaultString(infos.node.bitonestats) + ":" + w.conf.bitonestats
	}
