...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	...
	echo $'exec geth --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --bitonestats \'{{.Ethstats}}\' --cache=512 --http --http.addr=0.0.0.0 --http.corsdomain "*" --http.vhosts "*"' >> wallet.sh
...
func checkWallet(client *sshClient, network string) (*walletInfos, error) {
	...
	stats := &walletInfos{
		...
		webHost:  host,
		webPort:  webPort,
		bitonestats: infos.envvars["STATS"],
	}
	return stats, nil
}
