...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"bitonestats", "explorer", "wallet", "faucet"} {
	...
	// If we have bitonestats running, ask whether to make the secret public or not
	if w.conf.bitonestats != "" {
		fmt.Println()
		fmt.Println("Include bitonestats secret on dashboard (y/n)? (default = yes)")
