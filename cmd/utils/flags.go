...
// Package utils contains internal helper functions for go-bitone commands.
package utils
...
import (
	...
	"github.com/bitone/go-bitone/common/fdlimit"
	"github.com/bitone/go-bitone/consensus"
	...
	"github.com/bitone/go-bitone/eth/gasprice"
	"github.com/bitone/go-bitone/bitonedb"
	"github.com/bitone/go-bitone/bitonestats"
	"github.com/bitone/go-bitone/graphql"
	"github.com/bitone/go-bitone/internal/ethapi"
	"github.com/bitone/go-bitone/internal/flags"
	"github.com/bitone/go-bitone/les"
	"github.com/bitone/go-bitone/log"
	"github.com/bitone/go-bitone/metrics"
	"github.com/bitone/go-bitone/metrics/exp"
	"github.com/bitone/go-bitone/metrics/influxdb"
	"github.com/bitone/go-bitone/miner"
	...
	"github.com/bitone/go-bitone/p2p/netutil"
...
var (
	...
	EthStatsURLFlag = cli.StringFlag{
		Name:  "bitonestats",
		Usage: "Reporting URL of a bitonestats service (nodename:secret@host:port)",
	}
...
func SetShhConfig(ctx *cli.Context, stack *node.Node) {
	if ctx.GlobalIsSet(WhisperEnabledFlag.Name) ||
		...
		log.Warn("Whisper support has been deprecated and the code has been moved to github.com/bitone/whisper")
	}
}
...
// RegisterEthService adds an Bitone client to the stack.
func RegisterEthService(stack *node.Node, cfg *eth.Config) ethapi.Backend {
	if cfg.SyncMode == downloader.LightSync {
		backend, err := les.New(stack, cfg)
		if err != nil {
			Fatalf("Failed to register the Bitone service: %v", err)
		}
...
// RegisterEthStatsService configures the Bitone Stats daemon and adds it to
// the given node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := bitonestats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Bitone Stats service: %v", err)
	}
}
...
func MakeChainDatabase(ctx *cli.Context, stack *node.Node) bitonedb.Database {
	var (
		cache   = ctx.GlobalInt(CacheFlag.Name) * ctx.GlobalInt(CacheDatabaseFlag.Name) / 100
		handles = makeDatabaseHandles()

		err     error
		chainDb bitonedb.Database
	)
...
func MakeChain(ctx *cli.Context, stack *node.Node, readOnly bool) (chain *core.BlockChain, chainDb bitonedb.Database) {
	var err error
	chainDb = MakeChainDatabase(ctx, stack)
