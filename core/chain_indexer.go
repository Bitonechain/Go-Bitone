...
type ChainIndexer struct {
	chainDb  bitonedb.Database      // Chain database to index the data from
	indexDb  bitonedb.Database      // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb bitonedb.Database, indexDb bitonedb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
