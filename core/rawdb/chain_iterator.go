...
func InitDatabaseFromFreezer(db bitonedb.Database) {
...
func iterateTransactions(db bitonedb.Database, from uint64, to uint64, reverse bool) (chan *blockTxHashes, chan struct{}) {
...
func IndexTransactions(db bitonedb.Database, from uint64, to uint64) {
...
func UnindexTransactions(db bitonedb.Database, from uint64, to uint64) {
	// short circuit for invalid range
	if from >= to {
		return
	}
